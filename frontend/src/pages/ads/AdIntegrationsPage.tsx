...
const categoryLabels: Record<string, { label: string; color: string }> = {
  social: { label: 'Social Media', color: 'bg-slate-100 text-slate-600 border border-slate-200' },
  search: { label: 'Search', color: 'bg-slate-100 text-slate-600 border border-slate-200' },
  video: { label: 'Video', color: 'bg-slate-100 text-slate-600 border border-slate-200' },
  b2b: { label: 'B2B', color: 'bg-slate-100 text-slate-600 border border-slate-200' },
  scraping: { label: 'Web Scraping', color: 'bg-slate-100 text-slate-600 border border-slate-200' },
};
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-200">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h1 className="text-lg font-bold text-slate-900">Ad Platform Integrations</h1>
          ...
          <div className="flex items-center gap-4">
            <div className="text-center">
              <div className="text-xl font-bold text-primary-600">{connectedCount}</div>
              <div className="text-xs text-slate-500">Connected</div>
            </div>
            <div className="w-px h-8 bg-slate-200"></div>
            <div className="text-center">
              <div className="text-xl font-bold text-green-600">{platforms.length}</div>
              <div className="text-xs text-slate-500">Available</div>
            </div>
            <button
              onClick={loadIntegrations}
              className="ml-2 p-2 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors text-slate-600"
              title="Refresh"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition-colors text-xs font-medium"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-white rounded-lg p-3 border border-slate-200 hover:border-primary-200 transition-colors">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-primary-50 rounded-lg text-primary-600">
              <SignalIcon className="w-4 h-4" />
            </div>
            <div>
              <p className="text-sm text-slate-800 font-medium">Real-time Sync</p>
              <p className="text-xs text-slate-500">Webhooks for instant capture</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg p-3 border border-slate-200 hover:border-primary-200 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-800 font-medium">Scheduled Sync</p>
              <p className="text-xs text-slate-500">Auto-sync every 4 hours</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg p-3 border border-slate-200 hover:border-primary-200 transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-800 font-medium">Field Mapping</p>
              <p className="text-xs text-slate-500">Auto-map to CRM fields</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {platforms.map((platform) => {
          ...
          return (
            <div
              key={platform.key}
              className={`group relative bg-white rounded-xl border transition-all duration-200 hover:shadow-md flex flex-col ${
                hasIntegration ? 'border-green-300 bg-green-50/30' : 'border-slate-200 hover:border-primary-200'
              }`}
            >
              {/* Status Indicator */}
              {hasIntegration && (
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <div className="flex items-start gap-3 mb-3">
                  <div className={`p-2.5 rounded-xl ${platform.iconBg} text-white shadow-sm flex-shrink-0`}>
                    <div className="w-5 h-5 flex items-center justify-center [&>svg]:w-5 [&>svg]:h-5">
                      {platform.icon}
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-semibold text-slate-900 mb-1">{platform.name}</h3>
                ...
                <p className="text-xs text-slate-500 mb-4 leading-relaxed flex-1">{platform.description}</p>
      ...
      <div className="bg-white rounded-xl p-5 border border-slate-200">
        <h3 className="font-semibold text-slate-900 mb-4 text-sm">How It Works</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={item.step} className="flex items-start gap-3">
              <div className="flex-shrink-0 w-7 h-7 rounded-full bg-primary-600 text-white flex items-center justify-center font-semibold text-xs">
                {item.step}
              </div>
              <div>
